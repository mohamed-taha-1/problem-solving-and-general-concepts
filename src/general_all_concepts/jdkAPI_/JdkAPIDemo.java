...
public class JdkAPIDemo {

	/*
	 * <T> because all things in java are pointer to null; hence this will arises
	 * NULL-POINTER-EXCEPTION <T> FOR avoid that we will use OPTIONAL class.
	 */
	...
	private static void flatMapExample() {
		...
		name = personOptional.flatMap(User::getName).orElse("");
		System.out.println(name);
	}
	...
	private static void filterExample() {
		...
		System.out.println("filter() demo: "
				+ optional.filter(fullName -> fullName.contains("Andrey")).orElse("No required name found"));

		System.out.println("filter() demo: "
				+ optional.filter(fullName -> fullName.contains("Dima")).orElse("No required name found"));
	}
	...
	private static void orElseExample() {
		...
		System.out.println("orElse(): " + optional.orElse(JdkAPIDemo.getDefaultString()));
		System.out.println("orElseGet(): " + optional.orElseGet(JdkAPIDemo::getDefaultString));
